...
namespace DGP.ServiceLocator.Injectable
{
    public static class ObjectInjectorExtensions
    {
        ...
        private static T ConstructWithLocalServices<T>(object source)
        {
            ...
            foreach (var constructor in constructors) {
                ...
                for (int i = 0; i < parameterTypes.Length; i++) {
                    ...
                    resolvedParameterValue ??= sourceFields
                        .FirstOrDefault(field => field.fieldInfo.FieldType == parameterType).fieldInfo.GetValue(source);
                    resolvedParameterValue ??= sourceProperties
                        .FirstOrDefault(property => property.propertyInfo.PropertyType == parameterType).propertyInfo
                        .GetValue(source);
        ...
        private static void InjectFieldsToTarget<T>(object source, T target,
            (FieldInfo fieldInfo, ProvideAttribute provideAttribute)[] sourceFields)
        {
